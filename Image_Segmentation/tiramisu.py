...
K.set_image_dim_ordering('tf') # установка порядка измерений для картинок как в бэкэнде Theano


class Tiramisu():

    def __init__(self, nb_classes, img_dim, nb_dense_block=5, growth_rate=12, nb_filter=16, nb_layers=4,
                 upsampling_conv=128, bottleneck=False, reduction=0.0, dropout_rate=None, weight_decay=1E-4,
                 upscaling_type='deconv', verbose=True):
    ...
    def create(self):
        ...
        for block_idx in range(self.nb_dense_block):
            ...
            x, nb_filter = self.dense_block(x, self.nb_layers[-block_idx], nb_filter, self.growth_rate, bottleneck=self.bottleneck,
                                       dropout_rate=self.dropout_rate, weight_decay=self.weight_decay)
        x = Convolution2D(self.nb_classes, 1, 1, activation='linear', border_mode='same', W_regularizer=l2(self.weight_decay),
                          bias=False)(x)
        ...
        if self.verbose:
            print('Total number of convolutions', depth)
        if self.verbose:
